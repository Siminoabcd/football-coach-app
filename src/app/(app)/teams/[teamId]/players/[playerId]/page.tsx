...
export default async function PlayerProfilePage(
  ...
  // Performance stats for this player (all events)
  ...
  // Attendance for this player (all events)
  const { data: attend } = await sb
    .from("attendance")
    .select("event_id,status,rpe")
    .eq("player_id", playerId);

  // Fetch event metadata for recent list (limit to events we need)
  ...
  const rpeVals = (attend ?? [])
    .map(a => a.rpe)
    .filter((x): x is number => typeof x === "number");
  const avgRpe = rpeVals.length ? (rpeVals.reduce((s, v) => s + v, 0) / rpeVals.length) : null;

  // Recent performances (sort by event date desc if available, else updated_at)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <Link href={`/teams/${teamId}/players`} className="underline">← Back to players</Link>
      </div>
